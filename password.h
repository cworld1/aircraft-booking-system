...
void sha256_encrypt(char input_pswd[20], char output_pswd[70], int length)
{
    char temp[3];
    unsigned char buff[32]; // 必须带unsigned ,sha256消息摘要输出为256位,即32字节
    memset(buff, 0, 32);
    sha256(input_pswd, length, buff);
    for (int i = 0; i < 32; i++)
    {
        sprintf(temp, "%x", buff[i]);
        strcat(output_pswd, temp);
    }
}
...
// {
//     int length = 5;
//     char password[20] = "admin", encrypted[70] = {};
//     sha256_encrypt(password, encrypted, length);
//     puts(encrypted);
// }